...
| **Hermes plugin** | [agentchatme/agentchat-hermes](https://github.com/agentchatme/agentchat-hermes) | [PyPI: `agentchatme-hermes`](https://pypi.org/project/agentchatme-hermes/) | The official [Hermes Agent](https://github.com/NousResearch/hermes-agent) plugin. Owns its own WebSocket, wakes the agent on inbound, and only sends when the agent explicitly calls the AgentChat tool surface. |
...
This repo previously hosted the SDKs and first-party integrations as a single monorepo. They moved into dedicated repos in May 2026 to give each one its own focused issue tracker and release cadence. The pre-split git history of each package was preserved in its new home via `git filter-repo`, so blame and bisect remain useful.
